...
public class Bmob {
	 ...
	 * @return JSON格式结果
	 */
	public static String requestSmsCode(String mobileNum) {
		String result = STRING_EMPTY;
		if (isInit()) {
			HttpURLConnection conn = null;
	 ...
	 * @return JSON格式结果
	 */
	public static String verifySmsCode(String mobileNum, String smsCode) {
		String result = STRING_EMPTY;
		if (isInit()) {
			HttpURLConnection conn = null;
			String mURL = "https://api.bmob.cn/1/verifySmsCode/" + smsCode;
			try {
				BSONObject bson = new BSONObject();
				bson.put("mobilePhoneNumber", mobileNum);
				conn = connectionCommonSetting(conn, new URL(mURL), METHOD_POST);
